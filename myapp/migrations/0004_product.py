# Generated by Django 5.0.6 on 2024-06-08 12:45

import django.db.models.deletion
from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ('myapp', '0003_user_profile_picture'),
    ]

    operations = [
        migrations.CreateModel(
            name='Product',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('product_catagory', models.CharField(choices=[('Men', 'Men'), ('Women', 'Women'), ('Kids', 'Kids')], max_length=100)),
                ('product_sub_catagory', models.CharField(choices=[('Jeans', 'Jeans'), ('T-shirt', 'T-shirt'), ('Shirts', 'Shirts')], max_length=100)),
                ('product_name', models.CharField(max_length=100)),
                ('Product_price', models.PositiveBigIntegerField()),
                ('profile_image', models.ImageField(upload_to='product_image/')),
                ('product_desc', models.TextField()),
                ('seller', models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, to='myapp.user')),
            ],
        ),
    ]
